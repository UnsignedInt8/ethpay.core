...
export default class Payment {
  ...
  public static async send({
    ...
    value?: number | string;
    data?: string;
    currency: "eth" | "usdc" | "usdt" | "dai" | "sai" | "pax" | "tusd" | "gusd" | "usdx" | "husd" | "usdk";
  }) {
    if (!Metamask.hasWeb3()) {
      return "";
    }
    ...
    if (currency === "eth") {
      ...
      const contract = Contracts[currency] as {
        addr: string;
        decimals: number;
      };
      ...
      hash = await Metamask.sendTransaction({
        ...
        from,
        value: "0x00",
        data: Payment.buildErc20TransferABI(to, ethers.utils.parseUnits(`${value || 0}`, contract.decimals).toHexString())
      });
    }
